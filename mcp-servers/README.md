...
| 도구 | 입력 | 동작 | 어노테이션 |
| --- | --- | --- | --- |
| `archive_fetch` | `url`, `note?` | URL 본문 저장 + HTML 내 HWP/HWPX/PDF 링크 자동 다운로드 시도 | `readOnlyHint: true` |
...
- **로깅은 stderr** (`stdout`에 쓰면 JSON-RPC가 깨진다).
- read-only 도구는 `readOnlyHint`로 명시 — VS Code가 confirm을 생략한다.
- 쓰기 도구는 `chat.tools.eligibleForAutoApproval: false`로 두는 것을 권장한다.
- 자격증명은 항상 환경변수에서 읽고, 도구 인자/로그에는 마스킹한다.
