...
- `보관함/양식/` — 재사용 양식·템플릿·작성 안내·template guidance.
- `보관함/{legalize,precedent,admrule,ordinance}-kr/` — 법령·판례·행정규칙·조례 로컬 클론 (`scripts/fetch_legalize_kr.*`로 생성).

## 파일명 규칙

- `보관함/결과/`, `보관함/양식/`, `notebooks/`, `회고/` 등 사용자용 한국어 산출물은 **한글 파일명**.
- 날짜 접두어가 필요한 사용자용 산출물은 `<YYYY-MM-DD> <한글 파일명>.<ext>` 형태를 기본으로 한다.
- `보관함/결과/`는 `<YYYY-MM-DD> <한글 파일명>.md` 형태를 기본으로 한다.
- `보관함/양식/`의 재사용 템플릿·작성 안내는 날짜 접두어 없이 `<대상>-<용도>.md` 형태를 기본으로 한다.
- `보관함/다운로드/` 의 host·머신 식별 경로는 원어 그대로 유지 (예외).
- 코드·메타·로그·도구 설정은 영어 파일명.

## 폴더 구조 규칙

- `보관함/결과/`는 **flat**이 기본. 하위 폴더는 사용자가 명시하거나 이 파일에 별도 예외가 추가된 경우에만 사용.
- `보관함/양식/`도 **flat**이 기본. 재사용 양식, 작성 안내, guidance는 이 폴더에 직접 둔다.
